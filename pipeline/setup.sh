...
if command -v dnf >/dev/null; then
  sudo dnf install -y -q python3.11 python3.11-pip git gcc zlib-devel awscli >/dev/null 2>&1 || sudo dnf install -y -q python3 python3-pip git gcc zlib-devel
elif command -v apt-get >/dev/null; then
  sudo apt-get update -qq && sudo apt-get install -y -qq python3 python3-venv python3-pip git build-essential zlib1g-dev awscli
fi
...
pip install -q -e .
python - <<'PY'
import numpy, pyBigWig, zarr, Bio
print("deps ok:", "numpy", numpy.__version__, "zarr", zarr.__version__)
PY
