...
const GENE_ROW_H = 14, MAX_GENE_ROWS = 6, GENE_PAD = 8;
...
export function drawFigure(root: HTMLElement, v: FigureView, cb: FigureCallbacks = {}): FigureHandle | null {
  ...
  const geneH = geneRows ? geneRows * GENE_ROW_H + GENE_PAD * 2 : 0;
  const GROUPS_H = 26;
  const groupsH = (v.summary?.length ?? 0) ? GROUPS_H : 0;   // Fig 6d marginal: one dot per synteny group
  // colourbar lives top-left (the Groups track replaces a synteny legend)
  const CB_X = 8, CB_Y = 14, CB_H = 14;          // colourbar: taller bar, ticks below, then the "no alignment" swatch
  const legendH = CB_Y + CB_H + 16 + 22;
  const vGap = 20;   // room between the gene track and the heatmap for the TSS tick + arrow
  const top = Math.max(trackH * 2 + geneH + groupsH, legendH) + vGap;
  ...
  // Isolation is applied live: the persistent selection (click) is the base,
  // and hovering any group — legend, call, Groups dot — previews it on top.
  // Everything group-coloured registers here so a preview is a style pass, not a redraw.
  const band = svgEl("rect", { y: top, height: heatH, opacity: 0, "pointer-events": "none" }, svg);
  // guide lines while isolated: span start / end and mean position, white over
  // the heatmap and grey over the calls panel
  const guideG = svgEl("g", { "pointer-events": "none", opacity: 0 }, svg);
  const mkGuide = (stroke: string, w: number) => svgEl("line", { y1: top, y2: top + heatH, stroke, "stroke-width": w, "stroke-dasharray": "2 3", "stroke-linecap": "round" }, guideG);
  const heatGuides = [mkGuide("#fff", 1), mkGuide("#fff", 1), mkGuide("#fff", 1.5)];
  const callGuides = [mkGuide(DIM, 1), mkGuide(DIM, 1), mkGuide(FG, 1.5)];
  const groupEls: { el: Element; g: number; dimTo: number }[] = [];
  const applyIsolation = (preview: number | null) => {
    const eff = preview ?? selectedGroup;
    for (const { el, g, dimTo } of groupEls) el.setAttribute("opacity", eff === null || g === eff ? "1" : String(dimTo));
    const sp = eff !== null ? v.groupSpans?.get(eff) : undefined;
    if (sp) {
      band.setAttribute("x", String(x(sp.start))); band.setAttribute("width", String(Math.max(2, x(sp.end) - x(sp.start))));
      band.setAttribute("fill", groupColor(eff!, params.nMajor)); band.setAttribute("opacity", "0.18");
      const bps = [sp.start, sp.end, anchor.pos + sp.mean_dist_to_tss];
      bps.forEach((bp, i) => {
        for (const [ln, fx] of [[heatGuides[i], x], [callGuides[i], xCalls]] as [SVGLineElement, (bp: number) => number][]) {
          ln.setAttribute("x1", String(fx(bp))); ln.setAttribute("x2", String(fx(bp)));
        }
      });
      guideG.setAttribute("opacity", "1");
    } else { band.setAttribute("opacity", "0"); guideG.setAttribute("opacity", "0"); }
  };
  ...
  for (const c of calls) {
    ...
    const rect = svgEl("rect", {
      ...
      fill: groupColor(c.group ?? 0, params.nMajor), class: "f6e-call",
    }, cg) as HitRect;
    rect._call = c;
    groupEls.push({ el: rect, g: c.group ?? 0, dimTo: 0.12 });
  }
  ...
  // stats card for one synteny group (Fig 6d numbers), used by the legend and the Groups track
  const fmtDist = (d: number) => {
    const side = anchor.strand ? ((d < 0) === (anchor.strand === "+") ? "upstream" : "downstream") : (d < 0 ? "left of TSS" : "right of TSS");
    return `${Math.abs(d) < 1000 ? `${Math.round(Math.abs(d))} bp` : `${(Math.abs(d) / 1000).toFixed(1)} kb`} ${side}`;
  };
  const groupCard = (g: GroupSummary, isSel: boolean) => {
    const row = (k: string, val: string) => `<div class="f6e-kv-k">${k}</div><div class="f6e-kv-v">${val}</div>`;
    return `<div class="f6e-tip-title"><span class="f6e-tip-sw" style="background:${groupColor(g.group, params.nMajor)}"></span>Group ${g.group}` +
      `<span class="f6e-tip-dim"> · rank ${g.group} of ${v.summary?.length ?? 0} by enhancer count</span></div>` +
      `<div class="f6e-kv">` +
      row("Enhancers", `<b>${g.n_enhancers.toLocaleString()}</b>`) +
      row("Species", `${g.n_species} of ${nRows}`) +
      row("Mean GPS", g.mean_gps.toFixed(1)) +
      row("Mean distance", fmtDist(g.mean_dist_to_tss)) +
      row("Span", `${esc(anchor.chrom)}:${g.start.toLocaleString()}–${g.end.toLocaleString()} (${((g.end - g.start) / 1000).toFixed(1)} kb)`) +
      `</div><div class="f6e-tip-foot"><span class="f6e-hint">click to ${isSel ? "clear" : "isolate"} this group</span></div>`;
  };
  ...
  // colourbar (top-left, where the matplotlib figure keeps its legend)
  {
    ...
    for (const t of [0, 10, 20, 30]) {
      ...
      svgEl("text", { x: tx, y: cbY + cbH + 13, "text-anchor": t === 0 ? "start" : t === VMAX ? "end" : "middle" }, g).textContent = t === VMAX ? `≥${t}` : String(t);
    }
    ...
    // sits under the bar, left-aligned with the 0 tick
    const mx = cbX, my = cbY + cbH + 20;
    ...
    svgEl("rect", { x: mx, y: my, width: 14, height: 10, fill: params.showGaps ? "#ffffff" : VIRIDIS_FLOOR, stroke: DIM, "stroke-dasharray": params.showGaps ? "2 1" : "none" }, sw);
    svgEl("text", { x: mx + 18, y: my + 9, "text-decoration": "underline dotted" }, sw).textContent = "no alignment";
    svgEl("rect", { x: mx - 2, y: my - 3, width: 90, height: 16, fill: "transparent" }, sw);   // hit area
  ...
  const gy0 = trackH * 2 + groupsH + GENE_PAD;      // gene rows start below the Groups row
  // title aligned with the Coverage / Σ GPS axis titles (same x, colour and size)
  if (geneRows) svgEl("text", { x: heatX - 34, y: gy0 + (geneRows * GENE_ROW_H) / 2, "text-anchor": "end", "dominant-baseline": "middle", "font-size": 9, fill: DIM }, gg).textContent = "Genes";
  ...
  // Groups track (Fig 6d marginal): a dot per synteny group at its mean enhancer
  // position, sized by species count, coloured and numbered by group; a thin
  // line shows the group's span. Hover for the stats, click to isolate.
  if (groupsH) {
    ...
    for (const g of v.summary ?? []) {
      const cx = x(anchor.pos + g.mean_dist_to_tss);
      if (cx < heatX - 6 || cx > heatX + heatW + 6) continue;
      const rad = 3.5 + 6 * Math.sqrt(g.n_species / maxSp);
      const isSel = selectedGroup === g.group;
      const dot = svgEl("g", { class: "f6e-groupdot" }, grg);
      groupEls.push({ el: dot, g: g.group, dimTo: 0.3 });
      svgEl("line", { x1: Math.max(heatX, x(g.start)), y1: gy, x2: Math.min(heatX + heatW, x(g.end)), y2: gy, stroke: groupColor(g.group, params.nMajor), "stroke-width": 2, opacity: 0.6 }, dot);
      svgEl("circle", { cx, cy: gy, r: rad, fill: groupColor(g.group, params.nMajor), stroke: isSel ? FG : "#fff", "stroke-width": isSel ? 1.5 : 1 }, dot);
      if (rad >= 5) svgEl("text", { x: cx, y: gy, "text-anchor": "middle", "dominant-baseline": "central", fill: "#fff", "font-weight": 600, "pointer-events": "none" }, dot).textContent = String(g.group);
      else svgEl("text", { x: cx, y: gy - rad - 2, "text-anchor": "middle", fill: FG, "pointer-events": "none" }, dot).textContent = String(g.group);
      dot.addEventListener("mousemove", (ev) => { showTip(ev, groupCard(g, isSel), true); applyIsolation(g.group); });
      dot.addEventListener("mouseleave", () => { tip.hidden = true; applyIsolation(null); });
      dot.addEventListener("click", (ev) => { ev.stopPropagation(); cb.onSelectGroup?.(isSel ? null : g.group); });
    }
  }
  ...
  const hideTip = () => { tip.hidden = true; setHoverRow(null); hideTrackValues(); setGuides(null, "heat"); applyIsolation(null); cb.onHover?.(null); };
  ...
  cg.addEventListener("mousemove", (ev) => {
    ...
    if (!c) { tip.hidden = true; setHoverRow(null); applyIsolation(null); return; }
    applyIsolation(c.group || null);
